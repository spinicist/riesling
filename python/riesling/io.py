...
def _create_info(matrix, voxel_size, tr, origin, direction):
    info = np.array(
        [(matrix, tr, voxel_size, origin, direction)], dtype=INFO_DTYPE)
    return info
...
def write(filename, data, data_type='noncartesian', compression='gzip'):
    ...
    if not isinstance(data, xr.DataArray):
        # make it a DataArray with empty header and just assume that dimensions match
        data = xr.DataArray(data, attrs={}, dims=data_dims)
    ...
    if requires_traj:
        ...
        if data.attrs['trajectory'].ndim != 3:
            AssertionError(
                'Trajectory must be 3 dimensional (co-ords, samples, traces)')
    ...
    with h5py.File(filename, 'w') as out_f:
        ...
        if 'trajectory' in data.attrs:
            out_f.create_dataset('trajectory', data=data.attrs['trajectory'], chunks=np.shape(
                data.attrs['trajectory']), compression=compression)
        ...
        # write data
        out_f.create_dataset(data_type, dtype=dtype, data=data.data, chunks=np.shape(
...
def read(filename, dset=None):
    ...
    # try to load actual data
        ...
        if len(data.name) > 0 and data.name[0] == '/':
