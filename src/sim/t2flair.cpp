...
MultiFLAIR::MultiFLAIR(Settings const &s)
  : Sequence{s}
{
}

auto MultiFLAIR::length() const -> Index
{
  return settings.spg * settings.gps;
}

auto MultiFLAIR::parameters(Index const nsamp) const -> Eigen::ArrayXXf
{
  return Parameters::T1T2(nsamp);
}

auto MultiFLAIR::simulate(Eigen::ArrayXf const &p) const -> Eigen::ArrayXf
{
  float const T1 = p(0);
  float const T2 = p(1);
  float const R1 = 1.f / T1;
  ...
  Eigen::Matrix2f E1, E2, Eramp, Essi, Einv, Er, Erec;
  float const e1 = exp(-R1 * settings.TR);
  float const eramp = exp(-R1 * settings.Tramp);
  float const essi = exp(-R1 * settings.Tssi);
