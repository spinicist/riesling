...
Regularizers::Regularizers(RegOpts &opts, Sz4 const shape, std::shared_ptr<Ops::Op<Cx>> &A)
{
  ...
  if (opts.tgv) {
    ...
    auto prox1 = std::make_shared<Prox::SoftThreshold>(opts.tgv.Get(), op1->rows());
    auto grad_v = std::make_shared<GradVecOp>(grad_x->oshape);
    auto op2 = std::make_shared<Ops::Multiply<Cx>>(grad_v, ext_v);
    auto prox2 = std::make_shared<Prox::SoftThreshold>(opts.tgv.Get(), op2->rows());
    prox = {prox1, prox2};
    ops = {op1, op2};
  ...
  if (opts.wavelets) {
    ...
    prox.push_back(std::make_shared<Prox::ThresholdWavelets>(
      opts.wavelets.Get(), shape, opts.waveWidth.Get(), opts.waveLevels.Get()));
  }

  if (opts.llr) {
    ops.push_back(std::make_shared<Ops::Multiply<Cx>>(std::make_shared<TensorIdentity<Cx, 4>>(shape), ext_x));
    prox.push_back(std::make_shared<Prox::LLR>(opts.llr.Get(), opts.llrPatch.Get(), opts.llrWin.Get(), shape));
  }

  if (opts.nmrent) {
    ops.push_back(std::make_shared<Ops::Multiply<Cx>>(std::make_shared<TensorIdentity<Cx, 4>>(shape), ext_x));
    prox.push_back(std::make_shared<Prox::NMREntropy>(opts.nmrent.Get(), ops.back()->rows()));
  }

  if (opts.l1) {
    ops.push_back(std::make_shared<Ops::Multiply<Cx>>(std::make_shared<TensorIdentity<Cx, 4>>(shape), ext_x));
    prox.push_back(std::make_shared<Prox::SoftThreshold>(opts.l1.Get(), ops.back()->rows()));
  }

  if (opts.tv) {
    ops.push_back(std::make_shared<Ops::Multiply<Cx>>(std::make_shared<GradOp>(shape, std::vector<Index>{1, 2, 3}), ext_x));
    prox.push_back(std::make_shared<Prox::SoftThreshold>(opts.tv.Get(), ops.back()->rows()));
  }

  if (opts.tvt) {
    ops.push_back(std::make_shared<Ops::Multiply<Cx>>(std::make_shared<GradOp>(shape, std::vector<Index>{0}), ext_x));
    prox.push_back(std::make_shared<Prox::SoftThreshold>(opts.tvt.Get(), ops.back()->rows()));
  }
