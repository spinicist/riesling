...
{
  if (W == 3) {
    return std::make_unique<Grid<Scalar, Rectilinear<ND, ExpSemi<3>>>>(
      Mapping<ND>(traj, osamp, Rectilinear<ND, ExpSemi<3>>::PadWidth), nC, basis);
  } else if (W == 4) {
    return std::make_unique<Grid<Scalar, Rectilinear<ND, ExpSemi<4>>>>(
      Mapping<ND>(traj, osamp, Rectilinear<ND, ExpSemi<4>>::PadWidth), nC, basis);
  } else if (W == 5) {
    return std::make_unique<Grid<Scalar, Rectilinear<ND, ExpSemi<5>>>>(
      Mapping<ND>(traj, osamp, Rectilinear<ND, ExpSemi<5>>::PadWidth), nC, basis);
  } else if (W == 7) {
    return std::make_unique<Grid<Scalar, Rectilinear<ND, ExpSemi<7>>>>(
