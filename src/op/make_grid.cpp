...
template <typename Scalar, size_t ND>
auto make_grid(Trajectory const &traj, std::string const kType, float const osamp, Index const nC, Re2 const &basis)
  -> std::shared_ptr<GridBase<Scalar, ND>>
{
  if (kType == "NN") {
...
make_3d_grid(Trajectory const &traj, std::string const kType, float const os, Index const nC, Re2 const &basis)
{
  if (traj.nDims() == 2) {
