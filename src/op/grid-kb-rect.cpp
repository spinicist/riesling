...
{
  if (W == 3) {
    return std::make_unique<Grid<Scalar, Rectilinear<ND, KaiserBessel<3>>>>(traj, osamp, nC, basis);
  } else if (W == 4) {
    return std::make_unique<Grid<Scalar, Rectilinear<ND, KaiserBessel<4>>>>(traj, osamp, nC, basis);
  } else if (W == 5) {
    return std::make_unique<Grid<Scalar, Rectilinear<ND, KaiserBessel<5>>>>(traj, osamp, nC, basis);
  } else if (W == 7) {
    return std::make_unique<Grid<Scalar, Rectilinear<ND, KaiserBessel<7>>>>(traj, osamp, nC, basis);
  }
  Log::Fail("Invalid kernel width {}", W);
}

template auto make_kb_rect<Cx, 2>(
  Trajectory const &traj, size_t const W, float const osamp, Index const nC, std::optional<Re2> const &basis)
  -> std::unique_ptr<GridBase<Cx, 2>>;
template auto make_kb_rect<Cx, 3>(
  Trajectory const &traj, size_t const W, float const osamp, Index const nC, std::optional<Re2> const &basis)
