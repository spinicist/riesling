...
template auto
make_kb_rect<Cx, 2>(Trajectory const &traj, size_t const W, float const osamp, Index const nC, Re2 const &basis)
  -> std::shared_ptr<GridBase<Cx, 2>>;
template auto
make_kb_rect<Cx, 3>(Trajectory const &traj, size_t const W, float const osamp, Index const nC, Re2 const &basis)
