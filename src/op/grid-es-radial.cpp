...
{
  if (W == 3) {
    return std::make_unique<Grid<Scalar, Radial<ND, ExpSemi<3>>>>(
      Mapping<ND>(traj, osamp, Radial<ND, ExpSemi<3>>::PadWidth), nC, basis);
  } else if (W == 4) {
    return std::make_unique<Grid<Scalar, Radial<ND, ExpSemi<4>>>>(
      Mapping<ND>(traj, osamp, Radial<ND, ExpSemi<4>>::PadWidth), nC, basis);
  } else if (W == 5) {
    return std::make_unique<Grid<Scalar, Radial<ND, ExpSemi<5>>>>(
      Mapping<ND>(traj, osamp, Radial<ND, ExpSemi<5>>::PadWidth), nC, basis);
  } else if (W == 7) {
    return std::make_unique<Grid<Scalar, Radial<ND, ExpSemi<7>>>>(
