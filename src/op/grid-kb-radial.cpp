...
{
  if (W == 3) {
    return std::make_unique<Grid<Scalar, Radial<ND, KaiserBessel<3>>>>(
      Mapping<ND>(traj, osamp, Radial<ND, KaiserBessel<3>>::PadWidth), nC, basis);
  } else if (W == 4) {
    return std::make_unique<Grid<Scalar, Radial<ND, KaiserBessel<4>>>>(
      Mapping<ND>(traj, osamp, Radial<ND, KaiserBessel<4>>::PadWidth), nC, basis);
  } else if (W == 5) {
    return std::make_unique<Grid<Scalar, Radial<ND, KaiserBessel<5>>>>(
      Mapping<ND>(traj, osamp, Radial<ND, KaiserBessel<5>>::PadWidth), nC, basis);
  } else if (W == 7) {
    return std::make_unique<Grid<Scalar, Radial<ND, KaiserBessel<7>>>>(
