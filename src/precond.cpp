...
auto KSpaceSingle(Trajectory const &traj, Re2 const &basis) -> Re2
{
  ...
  weights.device(Threads::GlobalDevice()) = (weights * scale).inverse();
  weights.device(Threads::GlobalDevice()) = (weights > 1.f).select(weights.constant(1.f), weights);
  float const norm = Norm(weights);
  if (!std::isfinite(norm)) {
    Log::Print("Single-channel pre-conditioner norm was not finite ({})", norm);
  } else {
    Log::Print(
      "Single-channel pre-conditioner finished, scale {} norm {} min {} max {}",
      scale,
      norm,
      Minimum(weights),
      Maximum(weights));
  ...
  -> std::shared_ptr<Ops::Op<Cx>>
{
  if (type == "" || type == "none") {
    ...
    Re2 const              w = KSpaceSingle(traj, basis);
