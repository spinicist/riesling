...
struct UI
{
  ...
  void blitXYZ(RGBImage const &imgX, RGBImage const &imgY, RGBImage const &imgZ)
  {
    ...
    vopts.n = pX;
    auto ncvX = ncvisual_from_rgb_packed(imgX.data(), imgX.dimension(2), imgX.dimension(1) * 3, imgX.dimension(1), 255);
    ncvisual_blit(nc, ncvX, &vopts);
    ncvisual_destroy(ncvX);

    vopts.n = pY;
    auto ncvY = ncvisual_from_rgb_packed(imgY.data(), imgY.dimension(2), imgY.dimension(1) * 3, imgY.dimension(1), 255);
    ncvisual_blit(nc, ncvY, &vopts);
    ncvisual_destroy(ncvY);

    vopts.n = pZ;
    auto ncvZ = ncvisual_from_rgb_packed(imgZ.data(), imgZ.dimension(2), imgZ.dimension(1) * 3, imgZ.dimension(1), 255);
...
auto Colorize(Cx2 const &img, float const max, bool const grey) -> RGBImage
{
  ...
  for (int iy = 0; iy < nY; iy++) {
    for (int ix = 0; ix < nX; ix++) {
      ...
      std::array<float, 3> const &color = grey ? white : colorwheel[index];
...
int main_view(args::Subparser &parser)
{
  ...
  while (go) {
    ...
    ui.setStatus(fmt::format("{} B {}/{} I {}/{} J {}/{} K {}/{} V {}/{} Mag Lim {}", iname.Get(), iV, nV - 1, iI, nI - 1, iJ,
