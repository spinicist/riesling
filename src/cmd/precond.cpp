...
int main_precond(args::Subparser &parser)
{
  ...
  args::ValueFlag<float> threshold(parser, "T", "Threshold for inverse", {"thresh", 't'}, 0.f);
  ParseCommand(parser, trajFile);
  HD5::Reader reader(trajFile.Get());
  HD5::Writer writer(preFile.Get());
  writer.writeTensor(KSpaceSingle(Trajectory(reader), ReadBasis(basisFile.Get()), threshold.Get()), HD5::Keys::Precond);
  return EXIT_SUCCESS;
}
