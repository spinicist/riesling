...
auto Restarted(Vector const &b, Op::Ptr A, Op::Ptr P, std::vector<Regularizer> const &regs, Opts opts, Debug d) -> Vector
{
  return Restarted(CMap{b.data(), b.rows()}, A, P, regs, opts, d);
}

auto Restarted(CMap b, Op::Ptr E, Op::Ptr P, std::vector<Regularizer> const &regs, Opts opts, Debug debug) -> Vector
{
  if (b.rows() != E->rows()) { throw(Log::Failure("PDHG", "b had {} rows, expected {}", b.rows(), E->rows())); }
  if (regs.size() < 1) { throw(Log::Failure("PDHG", "Requires at least one regularizer")); }
  Index const                   nx = regs.size() + 1;
  ...
  for (Index ii = 0; ii < opts.imax; ii++) {
    ...
    for (Index ix = 0; ix < nx; ix++) {
      ...
      if (As[ix] && Ps[ix]) {
        ...
        ys[ix].device(Threads::CoreDevice()) += σ * x̂;
      }
      proxs[ix]->conj(σ, ys[ix]);
    }
    ...
    x̂.device(Threads::CoreDevice()) = 2.f * x - xold;

    /* Average iterations using running average formula */
    x̅.device(Threads::CoreDevice()) += (x - x̅) / (ii + 1);
    ...
    if (opts.adaptive) {
      ...
      Log::Print("PDHG", "{:02d}: |x̅| {:4.3E} |x| {:4.3E} |P| {:4.3E} |D| {:4.3E} σ {:4.3E} τ {:4.3E} α {:4.3E}", ii, normx̅,
      ...
      float const normdx = ParallelNorm(xold);
      float const rel = normdx / normx;
      Log::Print("PDHG", "{:02d}: |x̅| {:4.3E} |x| {:4.3E} |Δx|/|x| {:4.3E}", ii, normx̅, normx, rel);
      if (rel < opts.tol) {
        Log::Print("PDHG", "Δ tolerance reached");
        break;
      }
