...
namespace {
template <typename T> struct nifti_traits;

template <> struct nifti_traits<float>
{
  constexpr static int dtype() { return NIFTI_TYPE_FLOAT32; }
  constexpr static int bytesper() { return 4; }
};

template <> struct nifti_traits<std::complex<float>>
{
...
template <typename T, int ND> void WriteNifti(rl::Info const &info, Eigen::Tensor<T, ND> const &img, std::string const &fname)
{
  ...
  Log::Print("nifti", "Writing file {}", fname);
