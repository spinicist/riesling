...
{
  if (x.size() != y.size()) { throw Log::Failure("Interp", "Input vectors to spline must be same size"); }
  if (x.size() == 0) { throw Log::Failure("Interp", "Cannot create a spline with no control points"); }
  if (order < 1) { throw Log::Failure("Interp", "Interpolation order must be at least 1, was {}", order); }
  m_min = x[0];
