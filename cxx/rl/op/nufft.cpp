...
NUFFT<ND, KT>::NUFFT(Grid<ND, KT>::Opts const &opts, TrajectoryN<ND> const &traj, Index const nChan, Basis::CPtr basis)
  : Parent("NUFFT")
  ...
  , workspace{gridder->ishape}
{
  ...
  apo_ = Apodize<ND, KT>(FirstN<ND>(ishape), FirstN<ND>(gridder->ishape), opts.osamp).reshape(apo_shape); // Padding stuff
  Sz<InRank> padRight;
  padLeft_.fill(0);
  padRight.fill(0);
...
template <int ND, typename KT>
auto NUFFT<ND, KT>::Make(Grid<ND>::Opts const &opts, TrajectoryN<ND> const &traj, Index const nChan, Basis::CPtr basis)
  -> std::shared_ptr<NUFFT<ND>>
{
  return std::make_shared<NUFFT<ND>>(opts, traj, nChan, basis);
}

template <int ND, typename KT> void NUFFT<ND, KT>::forward(InCMap const &x, OutMap &y) const
{
...
template <int ND, typename KT> void NUFFT<ND, KT>::adjoint(OutCMap const &y, InMap &x) const
{
...
template <int ND, typename KT> void NUFFT<ND, KT>::iforward(InCMap const &x, OutMap &y) const
{
...
template <int ND, typename KT> void NUFFT<ND, KT>::iadjoint(OutCMap const &y, InMap &x) const
{
