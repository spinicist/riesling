...
template <int ND>
GridDecant<ND>::GridDecant(Grid<ND>::Opts const &opts, TrajectoryN<ND> const &traj, CxN<ND + 2> const &sk, Basis::CPtr b)
...
template <int ND>
auto GridDecant<ND>::Make(Grid<ND>::Opts const &opts, TrajectoryN<ND> const &t, CxN<ND + 2> const &skern, Basis::CPtr b)
...
void forwardTask(std::vector<typename TrajectoryN<ND>::CoordList> const &subs,
                 ...
                 typename KernelBase<Cx, ND>::Ptr const                 &kernel,
...
template <int ND> void GridDecant<ND>::forward(InCMap const &x, OutMap &y) const
{
  ...
  Threads::StridedFor(
    subs.size(), [&](Index const st, Index const sz) { forwardTask<ND>(subs, st, sz, subgridW, basis, kernel, skern, x, y); });
...
template <int ND> void GridDecant<ND>::adjoint(OutCMap const &y, InMap &x) const
{
  ...
  Threads::StridedFor(subs.size(), [&](Index const st, Index const sz) {
    adjointTask<ND>(subs, st, sz, mutexes, subgridW, basis, kernel, skern, y, x);
  });
