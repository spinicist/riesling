...
template <int Rank> Crop<Rank>::Crop(InDims const is, OutDims const os)
...
template <int Rank> auto Crop<Rank>::Make(InDims const is, OutDims const os) -> Ptr
{
  return std::make_shared<Crop<Rank>>(is, os);
}

template <int Rank> void Crop<Rank>::forward(InCMap x, OutMap y, float const s) const
{
  auto const time = this->startForward(x, y, false);
  y.device(Threads::TensorDevice()) = x.slice(left_, oshape) * y.constant(s);
  this->finishForward(y, time, false);
}
...
{
  auto const time = this->startAdjoint(y, x, false);
  x.device(Threads::TensorDevice()) = y.pad(paddings_) * x.constant(s);
  this->finishAdjoint(x, time, false);
}
