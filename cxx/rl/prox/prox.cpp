...
template <typename S> void Prox<S>::apply(std::shared_ptr<Op> const, CMap , Map ) const
{
...
template <typename S> void ConjugateProx<S>::apply(float const α, CMap x, Map z) const
{
  Vector x1 = x / α;
