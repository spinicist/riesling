...
auto SegmentedZTE::readout(float const R2, float const Δf) const -> Cx1
{
  if (p.samplesPerSpoke < 1) {
    Cx1 o(1);
    o(0) = Cx(1.f);
  ...
  // fmt::print(stderr, "Δf {} startPhase {} endPhase {}", Δf, startPhase, endPhase);
  return e;
}
...
auto SegmentedZTE::simulate(Eigen::ArrayXf const &pars) const -> Cx2
{
  if (pars.size() != nTissueParameters()) {
    throw Log::Failure("Sim", "Need {} parameters M0 T1 T2 Δf, received [{:.3f}]", nTissueParameters(), fmt::join(pars, ","));
  }
  float const M0 = pars(0);
  float const R1 = 1.f / pars(1);
  float const R2 = 1.f / pars(2);
  float const Δf = pars(3);
  ...
  Index           tp = 0;
  Cx1             s0(traces());
  Eigen::Vector2f Mz{m_ss * M0, 1.f};
  segment(tp, Mz, s0, R1, 1.f);
  if (tp != traces()) { throw Log::Failure("Sim", "Programmer error"); }
  return readout(R2, Δf).contract(s0, Eigen::array<Eigen::IndexPair<Index>, 0>());
}
