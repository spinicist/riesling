...
T2FLAIR::T2FLAIR(Pars const &s, bool const pt)
  : SegmentedZTE(s, pt)
{
}

auto T2FLAIR::nTissueParameters() const -> Index { return 4; }

auto T2FLAIR::simulate(Eigen::ArrayXf const &pars) const -> Cx2
{
  ...
  Eigen::Matrix2f const E2 = ET2p(R2, p.TE), V1 = inv(Q), V2 = inv(-Q), seg = Eseg(R1, 1.f);
...
auto T2FLAIR::timepoints() const -> Re1
{
  Re1   ts(traces());
  Index tp = 0;
  float t = 0.f;
