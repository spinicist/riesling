...
struct SegmentedZTE
{
  ...
  SegmentedZTE(Parameters const &s, bool const ps)
    : p{s}
    , preseg{ps}
  {
  }
  ...
  virtual auto simulate(Eigen::ArrayXf const &p) const -> Sim;
...
protected:
  ...
  void segment(Index &tp, float &t, Re1 &ts, Eigen::Vector2f &Mz, Cx1 &s0, float const R1, float const B1) const;
