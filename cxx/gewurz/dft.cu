...
void ThreeD::forward(DTensor<CuCx<TDev>, 3>::Span imgs, DTensor<CuCx<TDev>, 2>::Span ks) const
{
  ...
  thrust::for_each_n(thrust::cuda::par, it, nST,
                     [imgs, traj = this->traj, ks, scale] __device__(int st) {
                       ...
                       TDev const incx = ONE / SHORT_TO(nI);
                       TDev const incy = ONE / SHORT_TO(nJ);
                       TDev const incz = ONE / SHORT_TO(nK);

                       TDev rx = -FLOAT_TO(0.5f);
                       TDev ry = -FLOAT_TO(0.5f);
                       TDev rz = -FLOAT_TO(0.5f);
