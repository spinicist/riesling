                       ...
                       for (int ii = 0; ii < nI; ii++) {
                         for (int ij = 0; ij < nJ; ij++) {
                           for (int ik = 0; ik < nK; ik++) {
                             ...
                             ks(is, it) += ep * scale * imgs(ii, ij, ik);
                           }
                         }
                       }
                     });
      ...
      for (int is = 0; is < nS; is++) {
        float const p =
          pi2 * (traj(0, is, it) * (CuReal)(ii - nI / 2) / (CuReal)nI + traj(1, is, it) * (CuReal)(ij - nJ / 2) / (CuReal)nJ +
                 traj(2, is, it) * (CuReal)(ik - nK / 2) / (CuReal)nK);
        CuCxF const ep(cuda::std::cos(p), cuda::std::sin(p));
        imgs(ii, ij, ik) += ep * scale * ks(is, it);
      }
    }
  });
                       ...
                       for (int ii = 0; ii < nI; ii++) {
                         for (int ij = 0; ij < nJ; ij++) {
                           for (int ik = 0; ik < nK; ik++) {
                             ...
                             CuCxF const ep(cuda::std::cos(-p), cuda::std::sin(-p));
                             for (int ic = 0; ic < nC; ic++) {
                               ks(ic, is, it) += ep * scale * imgs(ic, ii, ij, ik);
                             }
                           }
                         }
                       }
...
{
  int const nC = imgs.extent(0);
  if (nC != ks.extent(0)) { throw rl::Log::Failure("DFT", "Image had {} channels k-space had {}", nC, ks.extent(0)); }
  int const   nI = imgs.extent(1);
  int const   nJ = imgs.extent(2);
  int const   nK = imgs.extent(3);
  int const   nIJ = nI * nJ;
  ...
  thrust::for_each_n(thrust::cuda::par, it, nIJK, [ks, traj = this->traj, imgs, scale] __device__(int ijk) {
    ...
    for (int it = 0; it < nT; it++) {
      for (int is = 0; is < nS; is++) {
        ...
        CuCxF const ep(cuda::std::cos(p), cuda::std::sin(p));
        for (int ic = 0; ic < nC; ic++) {
          imgs(ic, ii, ij, ik) += ep * scale * ks(ic, is, it);
        }
      }
    }
  });
