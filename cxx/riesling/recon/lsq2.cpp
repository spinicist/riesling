...
void main_recon_lsq2(args::Subparser &parser)
{
  ...
  ArrayFlag<float, 2>    cropFov(parser, "FOV", "Crop FoV in mm (x,y,z)", {"crop-fov"}, Eigen::Array2f::Zero());
  ...
  traj.checkDims(FirstN<2>(noncart.dimensions()));

  auto const basis = LoadBasis(coreArgs.basisFile.Get());
  auto const R = f0Args.Nτ ? Recon2(reconArgs.Get(), preArgs.Get(), gridArgs.Get(), senseArgs.Get(), traj, f0Args.Get(), noncart,
                                   reader.readTensor<Re3>("f0map"))
                           : Recon2(reconArgs.Get(), preArgs.Get(), gridArgs.Get(), senseArgs.Get(), traj, basis.get(), noncart);
  ...
  TOps::Pad<Cx, 5> oc(AddBack(traj.matrixForFOV(cropFov.Get()), R.A->ishape[2], R.A->ishape[3], R.A->ishape[4]), R.A->ishape);
